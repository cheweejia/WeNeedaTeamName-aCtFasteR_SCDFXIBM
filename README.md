...
First, you need to install the relevant nodes into your flow editor.

1. In Node-RED, under **manage palettes** (Click on the icon at the top right to open up a menu), install the following nodes:
- node-red-contrib-scx-ibmiotapp
- node-red-dashboard
- node-red-contrib-web-worldmap
- node-red-node-google
- node-red-contrib-momentCopy the following code.
...
```
3. In Node-RED, press **Ctrl-I** to open the import nodes dialog. Paste the code, click **OK**. Your imported flow should look like this: 
<img src="https://github.com/cheweejia/WeNeedaTeamName-aCtFasteR_SCDFXIBM/blob/master/assets/images/Flow%20imported.png" alt="Location of proximity sensor"></a>
4. Now you need to adapt the flow to your specific parameters. The only relevant parameters are the Device ID and inputting your own Google Maps API. Double-click the node IBM IoT App out. In the pop-up window, enter the Device ID that you used earlier (for example, 112233445566). 
5. To obtain a Google API key, go to <a href="https://console.developers.google.com/">Google Developer Console</a>. Create a new **Project**. Once done, go to the side menu on the right, click on **Credentials**. Then, click **Create Credentials** and select **API key**. Copy the API key. On Node-RED, double-click the node Google Geocoding. In the pop-up window, click the pen icon by the side of the Google API Key to enter the API key. 
6. Click **Deploy** in the flow editor. The flow is deployed and should be active immediately.
7. Click the dashboard tab (Looks like a chart) at the top right-hand corner. Then, click the button right below it to access your dashboard.
