...
Your imported flow should look like this: 
...
2. In Node-RED, press **Ctrl-I** to open the import nodes dialog. Paste the code, click **OK**. Under **manage palettes**, install the following nodes:
- node-red-contrib-scx-ibmiotapp
- node-red-dashboard
- node-red-contrib-web-worldmap
- node-red-contrib-moment
- node-red-node-google
3. Now you need to adapt the flow to your specific parameters. The only relevant parameters are the Device ID and inputting your own Google Maps API. Double-click the node IBM IoT App out. In the pop-up window, enter the Device ID that you used earlier (for example, 112233445566), and click **Import**. Double-click the node Google Geocoding. In the pop-up window, click the pen icon by the side of the Google API Key to edit. To obtain an API-key, go to <a href="https://console.developers.google.com/">Google Developer Console</a> and create a **Project**. Once done, go to the side menu on the left and click on **Credentials**. Then, click on **Create credentials** and select **API key**. Copy the generated API key into the Google geocoding node.
4. Click **Deploy** in the flow editor. The flow is deployed and should be active immediately.
5. Click the dashboard tab (Looks like a chart) at the top right-hand corner. Then, click the button right below it to access your dashboard.
